...
module Core {
  ...
  var checkKeycloakEnabled = function(callback: Function) {
    ...
    // Send ajax request to userDetails to figure out if keycloak integration is enabled
    $.ajax(userUrl, <JQueryAjaxSettings> {
      type: "GET",
      success: function (response: UserDetailsResponse) {
        log.debug("Got user response for keycloak context: ", response);
        var keycloakEnabled: boolean = response.keycloakEnabled;
  ...
  var createKeycloakContext = function(keycloakEnabled: boolean): KeycloakContext {
    ...
    // It's KeycloakClientConfigServlet, which handles to resolve keycloak.json on provided path
    var keycloakAuth: KeycloakModule.IKeycloak = new Keycloak('keycloak-config');
  ...
  export var keycloakLoginController = function($scope, jolokia, userDetails:Core.UserDetails, jolokiaUrl, workspace, localStorage, keycloakContext: KeycloakContext, postLogoutTasks) {
    ...
    // Now switch to true and allow controller to be processed again after 10 seconds
